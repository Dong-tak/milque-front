...
export function getConnectorPoints(
  from: RectangleShape | TextShape | SectionShape | BoardShape,
  to: RectangleShape | TextShape | SectionShape | BoardShape,
) {
  // 출발 도형과 도착 도형의 측면 중앙 좌표를 가져옴
  const fromCenters = getShapeSideCenters(from);
  const toCenters = getShapeSideCenters(to);

  // TextShape의 경우 width와 height가 없을 수 있으므로 기본값 설정
  const DEFAULT_WIDTH = 100; // 필요에 따라 조정
  const DEFAULT_HEIGHT = 50; // 필요에 따라 조정

  // 출발 도형의 중심 좌표 계산
  const fromCenterX =
    from.x +
    (isRectangle(from) ? from.width / 2 : (from.width ?? DEFAULT_WIDTH) / 2);
  const fromCenterY =
    from.y +
    (isRectangle(from) ? from.height / 2 : (from.height ?? DEFAULT_HEIGHT) / 2);

  // 도착 도형의 중심 좌표 계산
  const toCenterX =
    to.x + (isRectangle(to) ? to.width / 2 : (to.width ?? DEFAULT_WIDTH) / 2);
  const toCenterY =
    to.y +
    (isRectangle(to) ? to.height / 2 : (to.height ?? DEFAULT_HEIGHT) / 2);
  ...
  // 화살표 헤드를 객체 바깥으로 이동
  const arrowOffset = -10; // 화살표 헤드를 객체로부터 얼마나 떨어뜨릴지 결정하는 값
